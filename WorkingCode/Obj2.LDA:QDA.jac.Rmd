...
#LDA model build
mylda <- train(Subscription~ cons_price_idx + cons_conf_idx + euribor3m, data=banktrain, method="lda", trControl=fitControl)
#Predictions can come in many forms, the class form provides the categorical level of your response
Pred <- predict(mylda,newdata = banktest)
...
```{r qda}
myqda <- train(Subscription~ cons_price_idx + cons_conf_idx + euribor3m,data=banktrain, method="qda", trControl=fitControl)
#Predictions can come in many forms, the class form provides the categorical level of your response
PredQ <- predict(myqda,newdata = banktest)
